...
// ssh.go is the EMBEDDED ssh control server: the transport that exposes the control.Registry
// over ssh. It compiles ONLY under `-tags sshd`; the default build links neither it nor
// golang.org/x/crypto/ssh (ssh_stub.go provides the no-op seam). The commands themselves are
// transport-neutral and live in control_commands.go (default build); this file is just the
// ssh listener + key/auth config that serves them. See .notes/NEBULA-CONTROL-PLANE.md.

// sshControl aliases the embedded ssh control server.
type sshControl = *sshd.SSHServer

func newSSHControl(ctx context.Context, l *slog.Logger, reg *control.Registry) (sshControl, error) {
	return sshd.NewSSHServer(ctx, l, reg)
}
